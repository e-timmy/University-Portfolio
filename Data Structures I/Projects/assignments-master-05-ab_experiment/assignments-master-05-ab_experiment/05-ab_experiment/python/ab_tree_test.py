...
def audit(tree):
    ...
    def audit_node(node, parent, key_min, key_max, depth, leaf_depth):
        ...
        if not node:
            ...
            assert node.parent == parent, ("Inconsistent parent pointers ", node.keys, node.parent, parent)
            ...
            assert len(node.children) == len(node.keys) + 1, ("Number of keys does not match number of children in node ", node.keys)
...
def test_basic():
    ...
    for key in keys:
        ...
        show(tree)
        audit(tree)
        assert tree.find(key), ("Inserted key disappeared", key)
