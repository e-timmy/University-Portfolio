...
-- | Generate unique length combinations given list
uniqueCombos :: Int -> [a] -> [[a]]
uniqueCombos 0 _ = [[]]
uniqueCombos _ [] = []
uniqueCombos n (x:xs) = (map (x:) (uniqueCombos (n-1) xs)) ++ 
    (uniqueCombos n xs)
...
--    Logic behind Musician's chord guess
...
-- | Alternative guess optimisation 
--   Find size of most recurring feedback group for chord
